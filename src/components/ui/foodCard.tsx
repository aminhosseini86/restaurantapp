import tempFood from "@/assets/image/temp-food.png";
...
function FoodCard({
  ...
  const quantityChange = (action: "add" | "minus") => {
    setQuantityStock((prev) => {
      if (action === "add") {
        const number = prev === item.quantity ? item.quantity : prev + 1;
  ...
  return (
    ...
    >
      <div className="size-[120px]">
        <Avatar className="!rounded-8px size-full">
          <AvatarImage className="!rounded-8px" src={item.image} />

          <AvatarFallback className="rounded-8px">
            <img src={tempFood} className="rounded-8px" />
          </AvatarFallback>
        </Avatar>
      </div>

      <div className="flex flex-1 flex-col content-start items-start justify-between space-y-3">
        <h3 className="font-bold">{item.title}</h3>

        <p className="line-clamp-3 flex-1 text-sm">{item.description}</p>

        <p className="flex w-full content-center items-center justify-end">
          {item.price === 0 ? (
            "قیمت نامشخص"
          ) : (
            <>
      ...
      <div className="flex w-full content-center items-center justify-between">
        ...
        <div className="flex content-center items-center gap-1">
          <Button
            color={quantityStock === 0 ? "neutral" : "success"}
            variant="solid"
            className={clsx(
          ...
          {quantityStock !== 0 && (
            <>
              <input
                type="text"
                className="focus:border-p-red w-12 rounded-md border !border-gray-600 p-1.5 text-center text-sm font-semibold focus:outline-none disabled:bg-gray-100"
